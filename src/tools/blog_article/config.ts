...
export const config = {
    /** 是否启用模块 */
    enabled: (() => {
        const envEnabled = process.env.MODULE_BLOG_ARTICLE_ENABLED?.toLowerCase();
        const oldEnabled = process.env.TOOL_BLOG_ARTICLE_ENABLED?.toLowerCase();
    ...
    /** 任务超时时间（毫秒） */
    timeoutMs: parseInt(process.env.BLOG_ARTICLE_TIMEOUT_MS || '15000', 10),

    /** 最大并发数 */
    concurrency: parseInt(process.env.BLOG_ARTICLE_CONCURRENCY || '3', 10),
};
